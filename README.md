...
* [`WINDOW_TUNING.md`](WINDOW_TUNING.md) — the smoothing window and skew, swept over all four events with per-validator recovery hours measured from chain data. Headline: window length barely moves cliff events (it prices *sustained* outages), and among validators down from event onset a 24–36 h straggler pays 1.5x the 6–8 h crowd under this mechanism versus 4.7x under today's rules — the multiple over today's cost falls from ~57x for the fastest responders to ~9x for the slowest.
