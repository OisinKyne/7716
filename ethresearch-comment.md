*(Source of [this comment](https://ethresear.ch/t/supporting-decentralized-staking-through-more-anti-correlation-incentives/19116/18). All figures use GitHub URLs that Discourse rehosts on paste — the f-figures were regenerated for the adopted constants, so the old uploads must not be reused. Current vs live delta: smoothing window shortened to 2**16 throughout (constants, tables, figures) and the appended September note on the window revision.)*
...
tl;dr: under the original 7716 mechanism, a validator caught in a 10%-of-stake, 24 hour correlated outage loses $6.17, identical to an uncorrelated failure, whether the cap is 4 or 256. Under the proposed revision it loses ~$130 (about 3.6 weeks of rewards), the penalty factor scales linearly with event size up to the finality threshold, and a multi-day finality-losing outage reaches ~3.5% of principal in combination with the (unchanged) inactivity leak. Solo stakers failing on their own keep paying exactly what they pay today.
...
![Fig 1 — penalty factor trajectories](https://raw.githubusercontent.com/OisinKyne/7716/main/figures/f1_factor_trajectory.png)
...
![Fig 2 — cap invariance](https://raw.githubusercontent.com/OisinKyne/7716/main/figures/f2_cap_invariance.png)
...
Onset factors are then baseline-independent: ~9x at a 1% event, 39x at 5%, 78x at 10%, 154x at 20%, cap at one third — the factor discriminates by size across the whole band it is responsible for, and hands over to the leak exactly where the leak activates (Fig. 3).

![Fig 3 — onset factor vs event size](https://raw.githubusercontent.com/OisinKyne/7716/main/figures/f3_onset_vs_size.png)
...
| 1% correlated | 24 h | $6.17 | $19 (3.0x) | 3.6 d |
| 5% correlated | 24 h | $6.17 | $68 (11x) | 2 wk |
| 10% correlated | 6 h | $1.54 | $34 (22x) | ~1 wk |
| 10% correlated | 24 h | $6.17 | $130 (21x) | 3.6 wk |
| 10% correlated | 72 h | $18.50 | $142 (7.7x) | 4 wk |
| 20% correlated | 24 h | $6.17 | $253 (41x) | 7.1 wk |
| 40%, leak active | 24 h | $95 | $530 (5.6x) | 3.4 mo |
| 40% down 3 days (finality lost) | 72 h | $819 | $2,083 (2.5x) | ~13.5 mo ≈ 3.5% of principal |

Two notes on reading the table. In the 40% rows the total splits into this mechanism plus the *unchanged, pre-existing* inactivity leak: of the $2,083 three-day figure, $801 is the leak — the seemingly outsized numbers above one third are mostly the protocol's existing pricing, not this EIP. And the per-32-ETH framing understates what the operators being targeted would feel: the 10%/24h row is roughly 900 ETH per 1% of total stake an operator runs, on the order of 9,000 ETH for a 10%-of-stake operator whose fleet fails together for a day.

Fig. 4 compares mechanisms as a function of individual rectification time (the drafted mechanism literally overlaps the status quo line). Note the shape: exposure is meaningful but flattens once the cohort recovers, so the 72 hour straggler pays about $13 more than the 24 hour rectifier. Fig. 5 shows the same property as marginal cost per hour. Fig. 6 shows the layering with the leak across event sizes. (The clean gap-down at hour 24 in these charts is a modelling simplification; real events mostly do recover near-in-unison — a patch ships, operators restart — but if you model an exponentially staggered recovery instead, the aggregate anomaly persists a little longer and a 48h straggler pays ~10-25% more than the cliff model suggests, i.e. the cliff is the *optimistic* case for stragglers. Fig. 1 shows both. Nothing qualitative changes.)

![Fig 4 — cost vs rectification time](https://raw.githubusercontent.com/OisinKyne/7716/main/figures/f4_cost_vs_rectification.png)

![Fig 5 — marginal cost per hour](https://raw.githubusercontent.com/OisinKyne/7716/main/figures/f5_marginal_hours.png)

![Fig 6 — layering with the inactivity leak](https://raw.githubusercontent.com/OisinKyne/7716/main/figures/f6_leak_layering.png)

Here are some key bounds worth considering as we review this proposal. The worst-case penalty flow is ~0.75% of a 32 eth principal per day, and only while roughly a third of stake is newly offline and the validator itself is fully offline (for calibration: the leak takes ~50% in 18 days, slashing can take everything). A useful conversion at current rates is that X months of rewards ≈ X × 0.26% of principal, so even the harshest scenario in the table is nowhere near stake-level punishment. An uncorrelated validator coincidentally down for six hours during a 40% crisis pays ~$110, about three weeks of rewards — bounded, but a real cost of any statistical mechanism, since the protocol can't observe cluster membership. Flapping outages cost *less* than the same downtime taken as isolated events, because the elevated EMA acts as a refractory period. And after a major crisis the reference decays on its fixed half-life (Fig. 7), where a counter-based design with an equivalent budget would report factor ~1 for the better part of a year.

![Fig 7 — re-arming after a crisis](https://raw.githubusercontent.com/OisinKyne/7716/main/figures/f7_rearm.png)

My original gut-feel calibration was that a double-digit% liveness failure should cost "a month or more" to earn back. This proposal lands a 20%/24h event at ~7 weeks and a 10%/24h at ~3.7 weeks, which is about in range, without slamming into the cap at small event sizes (an earlier calibration I tried saturated at 5%, making a 10% event and a 30% event cost the same, which seems wrong). Also from [that 2024 comment](https://ethresear.ch/t/supporting-decentralized-staking-through-more-anti-correlation-incentives/19116/6): the suggestion that those offline during an outage should be hit harder than those who stay online through it — that's what this shape does, and the factor never drops below 1, so there are no discount windows subsidising missers during recovery.
...
- **One severity knob.** With the slope pinned to 3×(cap−1), `MAX_PENALTY_FACTOR` is the only free severity parameter: cap 256 gives the table above; cap 128 (slope 381, the calibration this analysis was first published at) halves the curve (10%/24h ≈ $69, about two weeks of rewards) with saturation still at one third. The ~6.3 day half-life separately sets how long the EMA remembers an event — the decay of an ongoing event's factor, the re-arm time, and (integrated out) the hard lifetime bound on what an extended outage can ever cost; see the September note below.
...
| --- | --- | --- | --- | --- |
| May 11+12 2023 finality incidents | 69%, cap binds | **1.16x** | **79x** | 1.9 d |
| Besu halt, Jan 2024 | 12.4% | 1.04x | **3.5x** | 4.3 h |
| Nethermind bug, Jan 2024 | 18.8% | 1.01x | **14x** | 21 h |
| Prysm post-Fusaka, Dec 2025 | 29.8% | 1.01x | **45x** | 4.7 d |

Two things worth pulling out. The drafted mechanism is a measured no-op on all four — including May 2023, its own motivating incident — which is the fixed-budget invariant on chain data rather than in algebra. And the revision is proportionate: 3.5x for a moderate incident, the cap binding only in the one event that crossed a third of stake. In operator terms, the December event prices at roughly 240 ETH per 1% of stake run — ~2,400 ETH for a 10%-of-stake operator caught for the full 4.5 hours, against low single-digit ETH today.

**The backtests also drove a recalibration.** At the calibration this comment originally proposed (cap 128), the worst five events of the Merge era priced at "a day or three" of income — real, but arguably light, and only because those events happened to be short. We swept caps 128–512 (slope tied at 3·(cap−1)) and slope-decoupled variants (steeper curve, cap held, saturation below one third) over the four events plus the worst-case scenarios. Cap 512 overreaches: a solo staker on the wrong majority client through a 3-day finality loss approaches 6% of principal, and an innocent bystander offline six hours in a crisis pays six weeks of income. The slope-decoupled variants hold every worst-case at the old level but price a 17% and a 30% event identically, giving up exactly the severity-proportionality the table above demonstrates. **Cap 256 (slope 765) was adopted**: it doubles the deterrent across the 1–33% band with every identity intact, and the numbers throughout this comment now reflect it. The full option table is in [SEVERITY.md](https://github.com/OisinKyne/7716/blob/main/SEVERITY.md).

Because the ingest records each validator's offline epochs individually, we could also measure something no postmortem documents: **cost as a function of each operator's own recovery time**. This addresses the long-tail concern directly. The chart below holds the population fixed to validators that went down at the original onset and stayed down until recovery — more validators went offline later (second waves, restart flapping), but their downtime landed after the factor collapsed and is priced near today's rates, so they're left out for simplicity. On the December 2025 event, an onset validator that took 24–36 hours to come back paid 1.5x what the 6–8 hour crowd paid; under *today's* rules that same spread costs 4.7x. Put the other way: the multiple over today's cost falls from ~57x for the fastest responders to ~9x for the slowest, because the charge lands on being correlated at onset, not on how long the fix takes. Besu's resync-stragglers — down two extra days — paid about twice today's cost.
...
On the revenue-neutrality departure flagged above, there's now a quantified answer to "isn't this an issuance debate by proxy" ([REDISTRIBUTION.md](https://github.com/OisinKyne/7716/blob/main/REDISTRIBUTION.md)): the worst events since the Merge would have burned ~4,600–6,000 ETH extra each — about half a percent of one year's issuance, once — and the steady-state burn is measurably zero. Redistributing instead would recreate either the fixed-budget no-op or a discouragement-attack bounty (the December event would have handed online validators a ~4,600 ETH pot, ~1,400 ETH of it to any 30% staker who *caused* a competitor's outage). Every penalty in the current spec burns; this one should too.
...
## Note, September 2026: smoothing window shortened to 2^16

One constants change since the update above, prompted by reviewers pushing on the extended-outage tail: **`OFFLINE_BALANCE_SMOOTHING_FACTOR` is now 2^16** (half-life ~6.3 days, was ~12.6), and every number in this post now reflects it.

The window sweep above showed the window barely moves real events — all of them were cliffs. What it didn't emphasise is that the window is also the *hard bound* on what an extended sub-finality outage can ever cost: the excess integral has a closed form, `slope · m · (26/64) · window/32` base rewards (the reference's time constant is `window` slots, but each validator is charged its factor once per epoch, on its one assigned slot — hence the /32), so at 2^17 a validator that joined a ⅓-of-stake outage and literally never came back was bounded at ~5.2 years of rewards. The same exponential that makes the mechanism front-loaded makes the window's marginal weeks the cheapest deterrence to trade away — so halving the window halves the worst tail at almost no cost to anything we measured:

| window | half-life | lifetime bound | ⅓ outage, worst ever | 10%×7d deterrent kept | Dec-2025 replay |
|---|---|---|---|---|---|
| **2^16 (adopted)** | **6.3 d** | **0.2·m of principal** | **~2.6 years of rewards** | 84% | 44.6x |
| 2^17 (previous) | 12.6 d | 0.4·m | ~5.2 years of rewards | 100% | 44.8x |

So the worst case the mechanism can now inflict — join a ⅓ outage, never return — is ~6.7% of principal, ever; a 30% event is bounded at ~6%, a 10% event at ~2%, however long the bug takes to patch, test and ship. Worth stating the consumer-hardware version too: solo and home stakers are collectively a low single-digit percent of stake, so even a failure that took down *every* consumer-grade setup indefinitely is a small event to the mechanism — a ~3% cohort draws a ~24x onset and a lifetime bound around 0.6% of principal, months of rewards.

The new value is not less anchored than the old one: 2^16 slots is exactly eight sync-committee periods and half the blob-sidecar retention window, and its half-life matches the weak subjectivity period after [EIP-8061](https://eips.ethereum.org/EIPS/eip-8061)'s churn increase lands in Glamsterdam (~7 days at current stake) — the mechanism now forgets an outage on roughly the timescale a node can stay offline and still trustlessly rejoin the chain. 8061 also matters for the exit route: at the new exit churn (~total/2^15 per epoch) a 30% cohort drains the queue in ~44 days, so the penalty bound and the cost of simply leaving converge — nobody is trapped paying multiples of what exiting costs. The full table, including the 2^15 option we held in reserve, is in [WINDOW_TUNING.md](https://github.com/OisinKyne/7716/blob/main/WINDOW_TUNING.md).
