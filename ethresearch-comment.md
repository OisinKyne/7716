...
The window sweep above showed the window barely moves real events — all of them were cliffs. What it didn't emphasise is that the window is also the *hard bound* on what an extended sub-finality outage can ever cost: the excess integral has a closed form, `slope · m · (26/64) · window` base rewards, so at 2^17 a validator that joined a ⅓-of-stake outage and literally never came back was bounded at ~5.2 years of rewards. The same exponential that makes the mechanism front-loaded makes the window's marginal weeks the cheapest deterrence to trade away — so halving the window halves the worst tail at almost no cost to anything we measured:
